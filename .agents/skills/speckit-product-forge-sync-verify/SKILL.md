...
category controls _auto-resolution eligibility_; severity controls _gate
behavior_.
...
Heuristic: if a drift item can change the _meaning_ of what any artifact
...
- **cosmetic count > drift_budget.cosmetic** → WARNING only. Reporter
  notes _"cosmetic budget exceeded ({count}/{budget}) — consider cleanup
  pass or raise the budget."_ The gate still passes.
